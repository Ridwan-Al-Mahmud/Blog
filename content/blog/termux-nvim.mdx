...
###### Nerd fonts
Before configuring NeoVim let's first configure our terminal. We will in install nerd-font in our local machine. This will provide us with beautiful dev-icons in a goody font. Write this command in your terminal.
```
...
```
***Don't change the the path ( ~/.config/nvim )***
...
```
What's this?? This is my custom Neovim configuration. What boring?!! 😑 You thought I was going to include the whole thing about how I configured my NeoVim?? Maybe in another blog. What's the best thing about it?? The code is written in **Lua** which is a functional script programming language that's well documented and the code is written in different portions so it is easy to edit anything you want in here. There's another popular choice kickstart.nvim. I will give the repo link [nvim-lua/kickstart.nvim](https://github.com/nvim-lua/kickstart.nvim). Now enter your NeoVim with  `vi .` and voila. Errors...?? You may get some errors after entering NeoVim for the first time. Mostly it will me about ***LSP( Language server protocol )*** it provides snippets, completions and much more. To fix this...
...
- If you followed me here and you want to install may like working with anyone in rust, go or typescript. Follow along me.
...
**Rust**.       
Installing rust may go complicated so I don't want to write the whole thing. Go to the documentation and install rust from the Linux ( [Rust-docs]( https://www.rust-lang.org/learn/get-started ) )         
**Go**                        
Install golang with....
```
apt install golang
...
```
Now run `Ctrl-r` and `Ctrl-I` this will install the necessary plugins. Check the there respective repositories to find there works. And last but not the least *options and remaps* checkout my custom remaps and options and add or remove anything you like.
Also checkout the config and plugins folder to understand what's going on.
***Saying that I'm going offline. See you later fellas***
